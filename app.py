...
if banco_file and app_file:
    ...
    if df_banco is not None and df_app is not None:
        try:
            ...
            col_ref_banco = find_column(df_banco, 'referencia') or find_column(df_banco, 'ref')
            col_monto_banco = find_column(df_banco, 'monto')
            col_tipo_banco = find_column(df_banco, 'tipomovimiento') or find_column(df_banco, 'concepto')
            col_fecha_banco = find_column(df_banco, 'fecha')
            
            # Para la app: necesitamos 'referencia' y 'monto (bs.)'
            ...
            if not col_ref_banco or not col_monto_banco:
                ...
                st.info(f"Columnas detectadas en el Banco: {list(df_banco.columns)}")
                st.stop()
            ...
            # Fix para el monto de la App: asegurar que se convierta correctamente (lidia con comas si las hay)
            ...
            for idx_app, row_app in df_app.iterrows():
                # FIX: Forzar a que la referencia de la fila sea explícitamente texto puro (str) 
                # y no un decimal 'float' como 'NaN' que genera error en la búsqueda.
                ref_app = str(row_app['Referencia_str']).strip().replace('.0', '')
                
                # Descartar filas basuras (vacías) que Pandas lee como "nan", "none" o sin texto
            ...
            # Quitar columna auxiliar monto_num_str si existe para no ensuciar el reporte
            ...
            if not df_diferencia_monto.empty and 'monto_num_str' in df_diferencia_monto.columns:
                df_diferencia_monto.drop(columns=['monto_num_str'], inplace=True)
            ...
            c4.metric("⚖️ Diferencia Monto", len(df_diferencia_monto))
            ...
            st.write(traceback.format_exc()) # Esto nos dirá exactamente la línea si vuelve a fallar