...
# Movemos find_column arriba para poder usarla en load_data
...
# Función robusta para cargar datos, elegir la segunda hoja y limpiar información
def load_data(file, is_app_file=False):
    try:
        file.seek(0)
        if file.name.lower().endswith('.csv'):
            ...
            # 1. Ajuste: Leer las hojas del archivo Excel
            xl = pd.ExcelFile(file)
            # Si el archivo tiene más de una hoja, seleccionamos la segunda (índice 1). Si no, la primera.
            target_sheet = xl.sheet_names[1] if len(xl.sheet_names) > 1 else xl.sheet_names[0]

            # Si es el archivo de la app, leemos primero para verificar si tiene el nuevo formato
            if is_app_file:
                xls_temp = pd.read_excel(file, sheet_name=target_sheet, nrows=5)
                # Si en la primera fila dice "CREDIABBY - REPORTE DE PAGOS", usamos header=4
                if not xls_temp.empty and any("CREDIABBY" in str(col).upper() for col in xls_temp.columns):
                    df = pd.read_excel(file, sheet_name=target_sheet, header=4)
                else:
        ...
        # 2. Ajuste: Filtrar únicamente la información de "Pago Móvil"
        col_metodo = find_column(df, 'método de pago') or find_column(df, 'metodo de pago')
        if col_metodo:
            # Filtramos buscando "pago m" para evitar problemas con acentos ("móvil" vs "movil") o mayúsculas
            mask = df[col_metodo].astype(str).str.lower().str.contains('pago m', na=False)
            df = df[mask].copy()
            # Restauramos el atributo que a veces se pierde al hacer un filtrado .copy() en pandas
            df.attrs['columnas_originales'] = columnas_originales
            
            if df.empty:
                st.warning(f"⚠️ Después de filtrar por 'Pago Móvil' en la segunda hoja de {file.name}, no quedaron registros.")
...
if banco_file and app_file:
    ...
    df_app = load_data(app_file, is_app_file=True)
    ...
    if df_banco_original is not None and df_app is not None:
        try:
            ...
            # Ajuste: Buscamos específicamente 'n° referencia' o 'referencia' en la app
            col_ref_app = find_column(df_app, 'n° referencia') or find_column(df_app, 'referencia') or find_column(df_app, 'ref')
            # Ajuste: Buscamos 'monto bs' o 'monto' en la app
            col_monto_app = find_column(df_app, 'monto bs') or find_column(df_app, 'bs') or find_column(df_app, 'monto')
            ...
            if not col_ref_app or not col_monto_app:
                ...
                st.write("Columnas detectadas en la app:", df_app.columns.tolist())
                st.stop()
            ...
            # --- Limpieza de la App ---
            # 1. Copiamos la columna a string
            df_app['Referencia_str'] = df_app[col_ref_app].astype(str).str.strip()
            
            # 2. Limpieza específica para las referencias manuales (Ej: "POS-PM-424283 (0102...")
            # Reemplazamos "POS-PM-" por vacío
            df_app['Referencia_str'] = df_app['Referencia_str'].str.replace('POS-PM-', '', regex=False)
            # Extraemos todo lo que esté antes del primer espacio o paréntesis de apertura usando una expresión regular
            df_app['Referencia_str'] = df_app['Referencia_str'].str.extract(r'^([^\s\(]+)', expand=False)
            # Finalmente quitamos el .0 si lo tuviera (para los números automáticos)
            df_app['Referencia_str'] = df_app['Referencia_str'].str.replace('.0', '', regex=False)
            ...
            # 2. Botón Excel Banco Resaltado en Amarillo
            with col_btn2:
