...
def load_data(file, is_app_file=False):
    try:
        ...
        if file.name.lower().endswith('.csv'):
            ...
            if is_app_file:
                xls_temp = pd.read_excel(file, nrows=5)
                ...
                if not xls_temp.empty and any("CREDIABBY" in str(col).upper() for col in xls_temp.columns):
                    file.seek(0)
                    xls = pd.read_excel(file, sheet_name=None, header=4)
                else:
                    file.seek(0)
...
if banco_file and app_file:
    ...
    if df_banco_original is not None and df_app is not None:
        try:
            ...
            # 1. Botón CSV usando pd.concat (Eliminado el .append() que causaba el error)
            frames_to_concat = []
            ...
            with col_btn1:
                if frames_to_concat:
                    # FIX: Aquí usamos pd.concat, esto soluciona el error de "AttributeError"
